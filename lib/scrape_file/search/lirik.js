...
const findGenius = async (q, h) => {
  ...
  const r = await axios.get(`https://duckduckgo.com/html/?q=${url}`, { headers: h });
...
const extractLyrics = (html) => {
  ...
  const clean = (f) => {
    ...
    // ekstra cleanup pakai cheerio lagi
  ...
  // fallback: PRELOADED_STATE
  const pre = html.match(/window.PRELOADED_STATE\s*=\s*({[\s\S]+?});/);
  if (pre) {
    try {
...
const ddgExtract = async (q, h) => {
  ...
  const r = await axios.get(`https://duckduckgo.com/html/?q=${qq}`, { headers: h });
  const $ = cheerio.load(r.data);
  let url = null;
  let snip = null;
  ...
  $('a').each((i, e) => {
    ...
    const grab = () => {
      const p = $(e).closest('.result,.result__body,.result__snippet,.c-result');
      snip =
        p.find('.result__snippet,.result__excerpt,.c-abstract,.snippet')
          .first()
          .text()
          .trim() || null;
      if (!snip)
...
const searchLyrics = async (q = '') => {
  ...
  try {
    ...
    // jika 403 / blocked → fallback DuckDuckGo
    if (e?.response?.status === 403) {
      const ddgUrl = await findGenius(q, hd);
      const { pageUrl, snippet } = await ddgExtract(q, hd);
      const link = pageUrl || ddgUrl;
      ...
      return {
        ...
        thumbnail: null,
        url: link || null,
        lyrics: text || (link ? `Preview mati, buka ${link}` : null)
      };
    }
    throw e;
  }
...
// default export untuk scrape('ai/lyrics')
